#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <math.h>
#include <raylib.h>
#include <omp.h>

#define SCREEN_W 640
#define SCREEN_H 480
#define NB_THREADS 8 // Nombre de threads explicite

// Seuils pour la détection des contours
#define UPPER_T 40
#define LOWER_T 10

typedef enum {
    HORIZONTAL, VERTICAL, L_DIAGONAL, R_DIAGONAL
} GradDir;

typedef enum {
    SUPPRESSED, WEAK, STRONG
} EdgePower;

typedef struct {
    GradDir dir;
    EdgePower power;
} PixelGrad;

Color *toGrayScale(Color *src_pixels, int img_w, int img_h) {
    Color *gray_pixels = calloc(img_w * img_h, sizeof(Color));

    #pragma omp parallel for schedule(static)
    for (int i = 0; i < img_h; ++i) {
        for (int j = 0; j < img_w; ++j) {
            Color pixel = src_pixels[img_w * i + j];
            unsigned char lum = (unsigned char)(0.299 * pixel.r + 0.587 * pixel.g + 0.114 * pixel.b);
            gray_pixels[img_w * i + j] = (Color){lum, lum, lum, pixel.a};
        }
    }
    return gray_pixels;
}

Color *gaussianBlur(Color *pixels, size_t img_w, size_t img_h) {
    float filter[3][3] = {
        {0.07511361, 0.1238414, 0.07511361},
        {0.1238414, 0.20417996, 0.1238414},
        {0.07511361, 0.1238414, 0.07511361}
    };
    
    Color *blured = calloc(img_w * img_h, sizeof(Color));

    #pragma omp parallel for schedule(dynamic)
    for (size_t i = 0; i < img_h; ++i) {
        for (size_t j = 0; j < img_w; ++j) {
            float val = 0;

            for (int k = -1; k <= 1; ++k) {
                for (int l = -1; l <= 1; ++l) {
                    if (i + k < 0 || i + k >= img_h || j + l < 0 || j + l >= img_w)
                        continue;
                    val += pixels[img_w * (i + k) + (j + l)].r * filter[1 + k][1 + l];
                }
            }

            unsigned char new_val = (unsigned char)fmin(fmax(val, 0), 255);
            blured[img_w * i + j] = (Color){new_val, new_val, new_val, pixels[img_w * i + j].a};
        }
    }

    return blured;
}

Color *detectEdge(Color *pixels, PixelGrad *grad_dir, int img_w, int img_h) {
    Color *y = calloc(img_w * img_h, sizeof(Color));

    float gx[3][3] = {{1, 0, -1}, {2, 0, -2}, {1, 0, -1}};
    float gy[3][3] = {{1, 2, 1}, {0, 0, 0}, {-1, -2, -1}};

    #pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < img_h; ++i) {
        for (int j = 0; j < img_w; ++j) {
            float xgrad = 0, ygrad = 0;

            for (int k = -1; k <= 1; ++k) {
                for (int l = -1; l <= 1; ++l) {
                    if (i + k < 0 || i + k >= img_h || j + l < 0 || j + l >= img_w)
                        continue;
                    xgrad += pixels[img_w * (i + k) + (j + l)].r * gx[1 + k][1 + l];
                    ygrad += pixels[img_w * (i + k) + (j + l)].r * gy[1 + k][1 + l];
                }
            }

            float theta = atan2f(ygrad, xgrad);
            double mag = sqrt(xgrad * xgrad + ygrad * ygrad);

            if (theta > 22.5 && theta <= 67.5) grad_dir[img_w * i + j].dir = R_DIAGONAL;
            else if (theta > 67.5 && theta <= 112.5) grad_dir[img_w * i + j].dir = VERTICAL;
            else if (theta > 112.5 && theta <= 157.5) grad_dir[img_w * i + j].dir = L_DIAGONAL;
            else grad_dir[img_w * i + j].dir = HORIZONTAL;

            unsigned char final_val = (unsigned char)fmin(fmax(mag, 0), 255);
            y[img_w * i + j] = (Color){final_val, final_val, final_val, pixels[img_w * i + j].a};

            if (mag >= UPPER_T) grad_dir[img_w * i + j].power = STRONG;
            else if (mag >= LOWER_T && mag < UPPER_T) grad_dir[img_w * i + j].power = WEAK;
            else grad_dir[img_w * i + j].power = SUPPRESSED;
        }
    }

    return y;
}

int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "Usage: ./sobel filename.png\n");
        return 1;
    }

    SetConfigFlags(FLAG_WINDOW_RESIZABLE);
    InitWindow(SCREEN_W, SCREEN_H, "Canny");

    Image src_image = LoadImage(argv[1]);
    int img_w = src_image.width, img_h = src_image.height;
    Color *pixels = LoadImageColors(src_image);

    // Définir explicitement le nombre de threads
    omp_set_num_threads(NB_THREADS);

    // Étapes de traitement parallélisées
    Color *gray_pixels = toGrayScale(pixels, img_w, img_h);
    Color *blured_pixels = gaussianBlur(gray_pixels, img_w, img_h);
    
    PixelGrad *grad_dir = calloc(img_w * img_h, sizeof(PixelGrad));
    Color *sobel_pixels = detectEdge(blured_pixels, grad_dir, img_w, img_h);

    Image gray_image = {.data = sobel_pixels, .width = img_w, .height = img_h,
                        .mipmaps = 1, .format = PIXELFORMAT_UNCOMPRESSED_R8G8B8A8};

    Texture2D texture = LoadTextureFromImage(gray_image);

    while (!WindowShouldClose()) {
        BeginDrawing();
        ClearBackground(WHITE);
        DrawTexture(texture, SCREEN_W / 2 - img_w / 2, SCREEN_H / 2 - img_h / 2, WHITE);
        EndDrawing();
    }

    UnloadTexture(texture);
    UnloadImageColors(pixels);
    free(gray_pixels);
    free(blured_pixels);
    free(sobel_pixels);
    CloseWindow();

    return 0;
}
nm